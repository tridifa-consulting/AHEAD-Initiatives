...
export default async function Home() {
  ...
  const workAreas = sections.filter(
    (s) => s.parent_slug === "work"
  );

  const publications = documents.filter(
    (d) => d.category === "publication"
  );

  const transparency = documents.filter(
    (d) => d.category !== "publication"
  );
  ...
  const registration = (
    bySlug["story"]?.extra?.registration ?? {}
  ) as Record<string, string>;
  ...
  const present = chapterOrder.filter(
    (slug) => bySlug[slug]
  );

  const chapters: NavChapter[] = present.map(
    (slug) => ({
      slug,
      label: navLabels[slug] ?? slug,
    })
  );

  const num = (slug: string) =>
    present.indexOf(slug) + 1;
    ...
    <>
      ...
      {bySlug["philosophy"] &&
        (() => {
          ...
          return (
            <Chapter
              slug="philosophy"
              number={num("philosophy")}
              title={t(bySlug["philosophy"].title)}
              tone="white"
            >
      ...
      {bySlug["work"] && (
        <Chapter
          slug="work"
          number={num("work")}
          title={t(bySlug["work"].title)}
          subtitle={t(bySlug["work"].subtitle)}
        >
      ...
      {bySlug["field"] && (
        <Chapter
          slug="field"
          number={num("field")}
          title={t(bySlug["field"].title)}
          subtitle={t(bySlug["field"].subtitle)}
          tone="white"
        >
          <div className="space-y-10">
            <Reveal>
              <Prose
                text={t(bySlug["field"].body)}
                className="max-w-3xl text-[#1F2933]/85"
              />
            </Reveal>
            ...
            <GalleryFilmstrip
              ...
              images={galleryFood.filter(
                (m) =>
                  m.title === "Home gardens" ||
                  m.title ===
                    "Natural resource management" ||
                  m.title === "Afforestation"
              )}
            />
      ...
      {bySlug["publications"] && (
        <Chapter
          slug="publications"
          number={num("publications")}
          title={t(bySlug["publications"].title)}
      ...
      {bySlug["blog"] && (
        <Chapter
          slug="blog"
          number={num("blog")}
          title={t(bySlug["blog"].title)}
          subtitle={t(bySlug["blog"].subtitle)}
        >
      ...
      {bySlug["media"] && (
        <Chapter
          slug="media"
          number={num("media")}
          title={t(bySlug["media"].title)}
          subtitle={t(bySlug["media"].subtitle)}
          tone="white"
        >
      ...
      {bySlug["reports"] && (
        <Chapter
          slug="reports"
          number={num("reports")}
          title={t(bySlug["reports"].title)}
          subtitle={t(bySlug["reports"].subtitle)}
          tone="ink"
        >
          <DocumentShelf
            items={transparency}
      ...
      {bySlug["partners"] && (
        <Chapter
          slug="partners"
          number={num("partners")}
          title={t(bySlug["partners"].title)}
          subtitle={t(bySlug["partners"].subtitle)}
          tone="white"
        >
          <PartnersGrid
            partners={partners}
      ...
      {bySlug["contact"] && (
        <Chapter
          slug="contact"
          number={num("contact")}
          title={t(bySlug["contact"].title)}
        >
          <div className="space-y-16">
            {t(bySlug["contact"].body) && (
              <Reveal>
                <Prose
                  text={t(bySlug["contact"].body)}
